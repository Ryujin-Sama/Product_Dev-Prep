...
Sol - Take 4 variables top bottom left right and run a while loop until left <= right and top <= bottom and run 4 for loops for each rows to print them in a spiral way
...
Sol - We use the same approach as 3 Sum problem but for this problem we keep i and j as constant and traverse between k and l with the same approach
...
Sol - We modify the merge sort algo and use two pointers to count the no of valid pairs, we do 2 sets of counting, one while dividing and second while we have two sets of sorted arrays, we combine both and return the ans.
